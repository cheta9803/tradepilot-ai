...
export class DashboardService {
    ...
    readonly summary =
        computed(() => this._summary());

    readonly indices =
        computed(() => this._indices());

    readonly positions =
        computed(() => this._positions());

    readonly orders =
        computed(() => this._orders());
    ...
    readonly scannerOpportunities =
        computed(
            () => this._scannerOpportunities(),
        );

    readonly aiOpportunities =
        computed(
    ...
    constructor() {
        ...
        // Refresh dashboard data automatically so
        // executed/closed trades appear without
        ...
        // Refresh dashboard recommendations every
        // 30 seconds during the NSE trading session.
        interval(30_000)
            .pipe(
                takeUntilDestroyed(this.destroyRef),
            )
    ...
    private updateLivePrices(): void {
        ...
        this._indices.update(
            indices =>

                indices.map(index => {

                    const tick =
                        this.marketStore.getTick(
                            index.name,
                        );
